...
class LocationProcessor(
    ...
    var totalPoints = 0; private set
    var originalDistanceM = 0.0; private set
    var cleanedDistanceM = 0.0; private set
    var abnormalDistanceM = 0.0; private set
    var mockDistanceM = 0.0; private set
     ...
     */
    fun process(fix: GpsFix, isPaused: Boolean, sensors: SensorSnapshot = SensorSnapshot()): ProcessResult? {
        val prev = last
        ...
        val row = LocationData(
            activity = if (isPaused) "PAUSED" else "DRIVING",
            speed = fix.speedMps,
            lat = fix.lat,
            lng = fix.lng,
            token = "", // set by the service before insert
            date = fix.timeMs,
            ...
            isAbnormal = abnormal,
            isPaused = isPaused,
            batteryPercentage = 0.0, // set by the service from BatteryManager
