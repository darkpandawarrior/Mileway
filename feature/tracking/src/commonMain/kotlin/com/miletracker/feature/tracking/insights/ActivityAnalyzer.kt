...
class ActivityAnalyzer {
    ...
    private fun analyzeActivityPatterns(
    ...
    ): Pair<Map<ActivityType, Double>, List<ActivityTransition>> {
        ...
        val percentages = activityDurations.mapValues { (_, d) ->
    ...
    private fun classifyActivity(point: LocationData): ActivityType = when {
    ...
    private fun analyzeAccelerationBehavior(points: List<LocationData>): AccelerationProfile {
        ...
        val total = (smoothAccelCount + harshAccelCount + smoothBrakeCount +
                harshBrakeCount + steadyCount).toDouble()
        return if (total > 0) {
            AccelerationProfile(
    ...
    private fun emptyProfile() = AccelerationProfile(
