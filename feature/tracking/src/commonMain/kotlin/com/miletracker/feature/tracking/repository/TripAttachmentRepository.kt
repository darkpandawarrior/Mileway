...
class TripAttachmentRepository(private val dao: TripAttachmentDao) {
    ...
    fun attachmentsForTrack(trackToken: String): Flow<List<TripAttachmentEntity>> =
        dao.observeForTrack(trackToken)

    /** Observe only receipt photos for a trip. */
    fun receiptsForTrack(trackToken: String): Flow<List<TripAttachmentEntity>> =
        dao.observeByType(trackToken, AttachmentType.RECEIPT)

    /** Save a receipt photo for the given trip. Returns the new row id. */
    suspend fun addReceipt(trackToken: String, uri: String): Long =
        dao.insert(
    ...
    suspend fun setOdometerStart(trackToken: String, uri: String, ocrText: String?): Long =
        dao.insert(
            TripAttachmentEntity(
                trackToken = trackToken,
    ...
    suspend fun setOdometerEnd(trackToken: String, uri: String, ocrText: String?): Long =
        dao.insert(
            TripAttachmentEntity(
                trackToken = trackToken,
    ...
    suspend fun getOdometerStart(trackToken: String): TripAttachmentEntity? =
        dao.getLatestOfType(trackToken, AttachmentType.ODOMETER_START)

    /** Latest odometer-end attachment, or null if none captured. */
    suspend fun getOdometerEnd(trackToken: String): TripAttachmentEntity? =
        dao.getLatestOfType(trackToken, AttachmentType.ODOMETER_END)

    /** All attachments for the track as a one-shot snapshot. */
    suspend fun getAll(trackToken: String): List<TripAttachmentEntity> =
        dao.getForTrack(trackToken)
