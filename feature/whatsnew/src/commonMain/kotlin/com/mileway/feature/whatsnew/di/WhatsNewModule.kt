...
/**
 * PLAN_V36 P1/P3/P4/P7 — `:feature:whatsnew`'s Koin module. The bundled repository binding, the
 * engagement recorder (offline outbox seam, spec §8), the list screen's ViewModel, and the detail
 * screen's ViewModel — the latter takes the nav-supplied entry id as a runtime parameter
 * (`viewModel { params -> ... }`), mirroring `feature:events`' `EventDetailViewModel` binding shape.
 */
val whatsNewFeatureModule =
    module {
        ...
        single { WhatsNewEngagementRecorder(get()) }
        viewModelOf(::WhatsNewListViewModel)
        viewModel { params -> WhatsNewDetailViewModel(entryId = params.get(), repository = get(), engagement = get()) }
    }
