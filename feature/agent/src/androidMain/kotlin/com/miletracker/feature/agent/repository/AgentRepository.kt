...
class AgentRepository {
    ...
    val conversations: List<AgentConversation> = AgentMockData.conversations.map { stub ->
        AgentConversation(
            id = stub.id,
            title = stub.title,
            lastMessageMs = stub.lastMessageMs,
            messages = stub.messages.map { msg ->
                AgentMessage(text = msg.text, isUser = msg.isUser, timestampMs = msg.timestampMs)
            }
        )
    }

    val popularQuestions: List<PopularQuestion> = AgentMockData.popularQuestions.map { stub ->
        PopularQuestion(
            id = stub.id,
    ...
    val unansweredQuestions: List<UnansweredQuestion> = AgentMockData.unansweredQuestions.map { stub ->
        UnansweredQuestion(
            id = stub.id,
            question = stub.question,
            module = stub.module,
    ...
    private val quickReplies: Map<String, String> = mapOf(
