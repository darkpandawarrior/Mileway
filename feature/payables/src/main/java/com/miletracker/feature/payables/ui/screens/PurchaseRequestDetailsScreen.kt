...
private fun PoHeaderCard(po: PurchaseOrder) {
    ...
    Card(
        ...
        Column(
            ...
            DetailRow("Total Amount", "₹%,.2f".format(po.totalAmount))
        }
    }
}
        ...
        Column(
            ...
            lineItems.forEach { item ->
                Row(
                    ...
                    Spacer(Modifier.size(DesignTokens.Spacing.l))
                    Text("₹%,.0f".format(item.lineTotal), style = MaterialTheme.typography.bodySmall, fontWeight = FontWeight.Medium)
                }
            }
            HorizontalDivider()
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text("Total (incl. GST)", style = MaterialTheme.typography.labelMedium, fontWeight = FontWeight.Bold)
                Text("₹%,.2f".format(total), style = MaterialTheme.typography.labelMedium, fontWeight = FontWeight.Bold)
            }
        }
    }
}
