...
object PolicyMockData {
    ...
    fun offices(): List<Office> = listOf(
        Office(
            code = "1345",
    ...
    fun businessEntities(): List<BusinessEntity> = listOf(
    ...
    fun outcomeFor(distanceKm: Double): SubmissionStatus = when {
     ...
     * empty list.
     */
    fun violationsFor(distanceKm: Double): List<PolicyViolation> = when (outcomeFor(distanceKm)) {
        SubmissionStatus.POLICY_VIOLATION -> listOf(
            PolicyViolation(
        ...
        SubmissionStatus.HARD_STOP -> listOf(
            PolicyViolation(
     ...
     */
    fun voucherFor(distanceKm: Double, token: String? = null): Voucher? {
     ...
     */
    fun transactionFor(distanceKm: Double, token: String? = null): TransactionRef? {
        if (outcomeFor(distanceKm) == SubmissionStatus.HARD_STOP) return null
        val seed = seedFor(distanceKm, token)
    ...
    fun voucherDeclaration(): VoucherDeclaration = VoucherDeclaration(
        text = "I declare that the distance claimed was travelled for business purposes " +
    ...
    fun enrich(
        ...
        distanceKm: Double,
        token: String? = null
    ): ExpenseSubmissionResponse = base.copy(
    ...
    private fun reimbursableAmountFor(distanceKm: Double): Double =
        min(distanceKm, MAX_REIMBURSABLE_KM_PER_DAY) * RATE_PER_KM
     ...
     */
    private fun seedFor(distanceKm: Double, token: String?): Long {
