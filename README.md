...
| Root Guard — signals detected | Root Guard — clean device | Tracking success |
|---|---|---|
| ![Root Guard screen showing detected root signals on a red background](docs/screenshots/root_guard_screen.png) | ![Root Guard screen for a clean device](docs/screenshots/root_guard_screen_clean.png) | ![Tracking success screen with journey summary and reimbursement details](docs/screenshots/tracking_success_screen.png) |

13 Roborazzi baseline images generated by `ScreenshotTests` (Roborazzi + Robolectric, JVM — no emulator needed). Run `fastlane screenshots` to refresh.
