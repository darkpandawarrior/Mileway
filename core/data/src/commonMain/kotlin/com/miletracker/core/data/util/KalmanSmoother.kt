...
class KalmanSmoother(private var processNoiseMetersPerSec: Double = 1.0) {
    ...
    fun reset() {
        initialized = false; lat = 0.0; lng = 0.0
        pLat = 1.0; pLng = 1.0; lastTimestampMs = 0L
    }

    fun setProcessNoiseMetersPerSec(mps: Double) { processNoiseMetersPerSec = mps.coerceAtLeast(0.0) }
    ...
    fun smooth(measuredLat: Double, measuredLng: Double, accuracyMeters: Float, timestampMs: Long): Pair<Double, Double> {
        ...
        if (!initialized) {
            lat = measuredLat; lng = measuredLng
            pLat = measNoiseLat * measNoiseLat; pLng = measNoiseLng * measNoiseLng
            lastTimestampMs = timestampMs; initialized = true
            return Pair(lat, lng)
        }

        val dtSec = max(0.0, (timestampMs - lastTimestampMs).coerceAtLeast(0L) / 1000.0)
        val processStd = processNoiseMetersPerSec * dtSec
        val qLat = metersToLatDeg(processStd); val qLng = metersToLngDeg(processStd, lat)

        pLat += qLat * qLat; pLng += qLng * qLng

        val rLat = measNoiseLat * measNoiseLat; val rLng = measNoiseLng * measNoiseLng
        val kLat = if (pLat + rLat > 0) pLat / (pLat + rLat) else 0.0
        val kLng = if (pLng + rLng > 0) pLng / (pLng + rLng) else 0.0

        lat += kLat * (measuredLat - lat); lng += kLng * (measuredLng - lng)
        pLat = (1 - kLat) * pLat; pLng = (1 - kLng) * pLng
    ...
    private fun metersToLngDeg(meters: Double, atLatDeg: Double): Double {
